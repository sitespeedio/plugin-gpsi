export function repackagePageSummary(psiResult) {
  ...
  const result = {
    score: {
      ...
      pwa: Number(
        Number(psiResult.lighthouseResult.categories.pwa.score * 100).toFixed(0)
      ),
  ...
  if (psiResult.loadingExperience.metrics) {
    ...
    if (psiResult.loadingExperience.metrics.EXPERIMENTAL_INTERACTION_TO_NEXT_PAINT) { 
      result.loadingExperience.EXPERIMENTAL_INTERACTION_TO_NEXT_PAINT = {
        percentile:
          psiResult.loadingExperience.metrics.EXPERIMENTAL_INTERACTION_TO_NEXT_PAINT.percentile,
        fast:
          psiResult.loadingExperience.metrics.EXPERIMENTAL_INTERACTION_TO_NEXT_PAINT
            .distributions[0].proportion,
        moderate:
          psiResult.loadingExperience.metrics.EXPERIMENTAL_INTERACTION_TO_NEXT_PAINT
            .distributions[1].proportion,
    ...
    if (psiResult.loadingExperience.metrics.EXPERIMENTAL_TIME_TO_FIRST_BYTE) { 
      result.loadingExperience.EXPERIMENTAL_TIME_TO_FIRST_BYTE = {
        percentile:
          psiResult.loadingExperience.metrics.EXPERIMENTAL_TIME_TO_FIRST_BYTE.percentile,
        fast:
